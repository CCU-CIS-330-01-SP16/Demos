...
namespace Lecture13Demos
{
    class Program
    {
        ...
        static void BinarySerialization<T>(AnimalList<T> list) where T : Animal
        {
            ...
            using (FileStream stream = File.Create("b-animals.txt"))
            {
                formatter.Serialize(stream, list);
            }

            AnimalList<T> deserializedList = null;
            using (FileStream reader = File.OpenRead("b-animals.txt"))
            {
        ...
        static void DataContractSerialization<T>(AnimalList<T> list) where T : Animal
        {
            ...
            using (XmlWriter writer = XmlWriter.Create("dc-animals.xml", settings))
            {
                serializer.WriteObject(writer, list);
            }

            AnimalList<T> deserializedList = null;
            using (XmlReader reader = XmlReader.Create("dc-animals.xml"))
            {
        ...
        static void DataContractJsonSerialization<T>(AnimalList<T> list) where T : Animal
        {
            ...
            using (FileStream fileStream = File.Create("dcj-animals.txt"))
            {
                serializer.WriteObject(fileStream, list);
            }

            AnimalList<T> deserializedList = null;
            using (FileStream reader = File.OpenRead("dcj-animals.txt"))
            {
                deserializedList = serializer.ReadObject(reader) as AnimalList<T>;
            }
        ...
        static void NewtonsoftJsonSerialization<T>(AnimalList<T> list) where T : Animal
        {
            ...
            using (StreamWriter writer = File.CreateText("nsj-animals.txt"))
            {
                serializer.Serialize(writer, list);
            }

            AnimalList<T> deserializedList = null;
            using (StreamReader reader = File.OpenText("nsj-animals.txt"))
            {
